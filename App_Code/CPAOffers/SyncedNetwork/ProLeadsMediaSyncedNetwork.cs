...
{
    public class ProLeadsMediaSyncedNetwork : SyncedNetwork
    {
        public ProLeadsMediaSyncedNetwork(AffiliateNetwork Network, bool ThrowExceptions = false) : base(Network, ThrowExceptions) { }        

        public static void SynchronizeAll(bool ThrowExceptions = false)
        {
            var where = TableHelper.MakeDictionary("AffiliateNetworkSoftwareTypeInt", (int)AffiliateNetworkSoftwareType.ProLeadsMedia);
            ...
            {
                var SN = new ProLeadsMediaSyncedNetwork(elem);
                SN.Synchronize();
            }
        }
        ...
        private static JArray GetElement(string apiKey, bool ThrowExceptions = false)
        {
            try
            {
                ...
                {
                    var data = c.DownloadString(string.Format("http://proleadsmedia.afftrack.com/apiv2/?key={0}&action=offers&format=json&limit=1652", apiKey));
        ...
        private static string GetGeolocation(string apiKey, string offerId)
        {
            ...
            try
            {
                using (MyWebClient c = new MyWebClient())
                {
                    var data = c.DownloadString(string.Format("http://proleadsmedia.afftrack.com/apiv2/?key={0}&action=offer_countries&format=json&id={1}", apiKey, offerId));
        ...
        private static JObject GetCreatives(string apiKey, string offerId)
        {
            try
            {
                using (MyWebClient c = new MyWebClient())
                {
                    var data = c.DownloadString(string.Format("http://proleadsmedia.afftrack.com/apiv2/?key={0}&action=offer_creatives&format=json&pid={1}", apiKey, offerId));
